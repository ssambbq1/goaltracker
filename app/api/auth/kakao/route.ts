...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: "kakao",
      options: {
        redirectTo: `${origin}/api/auth/kakao/callback`,
      },
    });

    if (error) throw error;
    if (!data.url) throw new Error("Kakao login URL was not returned.");
    ...
    const message = getErrorMessage(error, "Failed to start Kakao login");
