...
import { applySessionCookie, getErrorMessage, loginWithKakaoUser } from "@/lib/auth";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!code) throw new Error("Kakao login code is missing.");
    ...
    if (!data.user) throw new Error("Kakao user was not returned.");

    const loginId = await loginWithKakaoUser(data.user);
    ...
    const message = getErrorMessage(error, "Failed to finish Kakao login");
